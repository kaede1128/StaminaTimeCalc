...
var date4google, getDateStr, getLink4google, getLinkTore4google, getPatternId, getTime, getToretikeDate, sec2HourMin, update, updateTore, zerofill,
...
update = function() {
  var atStr, inputDate, inputTime, maxDate, maxStm, nowDate, nowStm, reqSec, reqStr;
  updateTore();
...
updateTore = function() {
  var d, dates, groupId, i, len, nowDate, toreAtStr, toreGoogleTime, toreReqStr, toreTime;
  nowDate = new Date();
  toreReqStr = '';
  toreAtStr = '';
  toreGoogleTime = 0;
  ...
  if (groupId >= 0) {
    ...
    for (i = 0, len = dates.length; i < len; i++) {
      ...
      if ((-1000 * 60 * 60 < toreTime && toreTime <= 0)) {
        ...
        toreAtStr += d.getDate() !== nowDate.getDate() ? '明日 ' : '今日 ';
